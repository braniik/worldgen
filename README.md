...
| Key   | Action                          |
|-------|---------------------------------|
| `G`   | Generate a new world (next seed) |
...
- **Stateless hashing.** `wg_hash2(seed, x, y)` gives the same answer no
  matter when or in what order it's called, so any tile can be regenerated
  in isolation. Reason is chunked or potential infinite worldgen in the future maybe.
...
And I don't want that, so therefore this so solution is: any tile can be computed on its own, in any order, on any thread, without touching the others.
...
- **It avalanches.** Flip one bit of the input and every output bit flips with probability ~1/2. This is the Strict Avalanche Criterion. Bijection alone is not enough, which surprised me. 
